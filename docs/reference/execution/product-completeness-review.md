---
id: REF-PRODUCT-COMPLETENESS-REVIEW
title: University 商业产品完整性检查与改版图示
type: reference
status: active
canonical: true
owner: ai-assisted
...
请打开 [改前／改后图文对照](product-before-after/before-after.md)，或双击它同目录的
[浏览器阅读版](product-before-after/index.html)。那里按页面排列了改前、改后截图，
也标明原先没有、后来新增的位置。本页保留原始问题和提案，不再重复维护实施后的图集。
...
本文是这次检查的证据和待讨论建议，不是已批准的新产品规则。实现前，涉及首次体验、学习完成、免费与付费边界的决定，应回到 [玩家旅程 V5](../player-journey/v5/index.html) 确认。当前价格和权益以代码为准，本文不另建价格表。
...
| 先后 | 现在最值得做什么 | 对应事项 |
| --- | --- | --- |
| 第一批：先保住信任 | 分清答错与不能判断；保住未提交答案；说清保存状态；不宣传未兑现的能力 | 10、11、19、20 |
| 第二批：让第一次来的人学到东西 | 欢迎互动、目标选择、第一小课、清楚的下一步 | 01–05、08、12、15 |
| 收费开关打开前必须完成 | 所选周期进入订单；支付与权益到账；管理订阅与取消；费用、服务说明、客服 | 21–25 |
| 随后补齐持续使用 | 练习有终点；成长页说人话；提醒送达；无障碍与衡量改版效果 | 06–07、09、13–18、26–30 |
...
| 轮次 | 走法 | 得到的证据 |
| --- | --- | --- |
| 第一轮：当新用户 | 全新访客，逐一打开首页、选课、目录、复习、练习、任务、排行榜、会员、个人档案、设置、图鉴、试玩场 | 12 个入口 × 电脑与手机两种尺寸，共 24 组截图和页面记录 |
| 第二轮：真正点击 | 从首页进入第一节；切换会员周期并点击购买入口；打开反馈；尝试提醒；检查减少动态效果和课程加载失败 | 交互截图、4 个页面的自动无障碍检查、明确标注的故障注入 |
| 第三轮：找边角与反例 | 写答案后刷新；实际提交一段改写；确认阅读并跳到下一节；检查任务按钮；补看 320 宽小屏；撤销网络故障后点击重试 | 答案消失前后对照、判题结果、能够继续学习和恢复加载的证据 |
...
完整原始图片和页面记录在 [证据目录](product-review-evidence/)。每张截图旁边的同名 JSON 保存地址、视口、时点和页面文字。`capture-index.json`、`interaction-index.json`、`third-pass-index.json` 分别对应三轮。

## 先看“现在是什么样”

### 电脑：首页像工作台，而不像第一次来时的邀请

![真实截图：电脑第一次进入 University](product-review-evidence/desktop-first-visit.png)

页面确实有“开始学习”，不能说用户无路可走。但第一次来，就要同时理解地图、课程系列、图鉴、练习、任务、排行榜、会员、个人档案和经验值。“我为什么要在这里学？”没有被直接回答。

### 手机：开始按钮清楚，前后的故事还没接上

![真实截图：手机第一次进入 University](product-review-evidence/phone-first-visit.png)

这个橙色大按钮应该保留。问题不是再做一个更大的按钮，而是用户点之前是否知道会得到什么，点之后是否很快体会到“这里确实能让我学会”。底部七个入口不等于程序错误，但对第一次来的人有压迫感，是需要验证的设计取舍。

### 第一节：合理改写得到“未通过”

![真实截图：输入一段自己的解释后，出现未通过](product-review-evidence/phone-answer-result.png)

本次输入是：“手机先把当前的脸算成一串特征数字，再和设置解锁时保存的那串比较，差得够少就通过，不是把两张照片逐张对比。”它表达了这节课要求的核心意思。页面却显示“当场判定 · 未通过”，接着要求用户再想一下。问题不在于必须替每个答案判对，而在于 **没有判断能力时，不应该给出已经判断错误的口气**。

### 会员页：价格已经摆出来，完整购买服务还没接好

![真实截图：当前会员页面](product-review-evidence/desktop-plans.png)

美元是现有海外定价选择，本身不是错误。真正重要的是：用户能否知道今天能不能买、买的到底是什么、会不会另扣钱，以及以后在哪里取消。

## 开场应该怎么做

![改版结构示意：欢迎、选择、动手、进入第一课](product-review-evidence/proposal-onboarding.svg)

**建议主题：小岛不是装饰，而是“我学会一件事”的可见结果。** 沿用现在的奶油色面板、橙色动作按钮、柔和天空和小伙伴，不另外造一个与学习界面无关的霓虹广告片。
...
![改版结构示意：今天只突出一个学习动作，完成后给真实收获](product-review-evidence/proposal-learning.svg)

进入后只有一件最重要的事：继续上次学习，或完成今天到期的复习。选课、图鉴、会员都能找到，但不需要每次争第一眼。

完成后说“你今天做成了什么”，而不只是“+XP”。例如：“你能解释按钮点击后，页面为什么会更新了；还有一处关于请求失败的判断明天回来再试。”示例必须由实际完成记录和题目能力映射产生，不能用漂亮的假总结代替证据。

## 收费完整性的图，比多一张价格卡更重要

![改版结构示意：从购买到取消的服务闭环](product-review-evidence/proposal-billing.svg)

可以先把产品作为免费体验开放，但**收费开关应由整条服务链决定，而不是由价格卡已经画好决定**。浏览器只展示服务端确认的订单和权益；共用的账号、钱包、支付能力继续归 SwimmerBackend。退款和取消政策是需要负责人确认的商业决定，本轮没有擅自决定。
...
| 范围 | 建议归属 | 这次不做什么 |
| --- | --- | --- |
| 欢迎流程、选课入口、状态文案、学习路径、付款前展示 | University-product | 不自行另造课程内容或改变他人视觉分支 |
| 第一小课、观察实验、能力目标、题目判定材料 | University-courses | 不把旧课批量标为合格；与产品确定第一价值体验接口 |
| 动画、岛屿和镜头的品牌表现 | University-visual，与产品共享交互要求 | 不让开场成为加载与性能的新负担 |
| 通用按钮、对比度 token、无障碍基础能力 | SwimmerUIKit | 不在产品复制另一套控件 |
| 账号、同步权益、订单、取消、真实推送 | SwimmerBackend 及既有共享能力 | 不让浏览器自己发权益或改钱包 |
...
| 官方资料 | 本文用在哪里 | 地址 |
| --- | --- | --- |
| Apple Human Interface Guidelines：Onboarding | 简短、可选、在操作中理解，不堆启动配置 | `https://developer.apple.com/design/human-interface-guidelines/onboarding` |
| W3C：Understanding SC 2.2.2 Pause, Stop, Hide | 自动运动内容不能成为无法控制的打扰；该准则有持续时间、并行内容等适用条件 | `https://www.w3.org/WAI/WCAG22/Understanding/pause-stop-hide.html` |
| Paddle：Customer portal integration | 可复用客户门户管理支付方式、账单与订阅，不自行重造 | `https://developer.paddle.com/build/customers/integrate-customer-portal/` |
| Paddle：Cancel subscriptions | 取消下次续费与立即终止是不同状态，要随服务端事件更新权益 | `https://developer.paddle.com/build/subscriptions/cancel-subscriptions/` |
