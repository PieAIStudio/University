...
价格是代码配置，不在本页再抄一份金额或把配置当作正式商业承诺。会员的结构化
批改仍消耗钱包；免免费日额度上限不等于会员价格包含无限使用。开放辅导的计划
...
- 无订单通道时 CTA 为“记录购买意向”，打开能力说明，不下单、不扣款；
  未登录先说明账号要求，匿名身份先绑定邮箱。入口保持可见。
- 浏览器用加密 UUID 并在同页复用同一订单请求；同订单换 offer 被拒。
  这是交互去重，不是跨刷新、跨设备或伪造请求的最终幂等保障。
- 月／年选择目前只改变展示价，购买仍传 `plan.id`（`member`）。
  真正接线时必须明确映射到服务端不同报价，不让前端自报金额。
- `PaymentPort` 没有订阅取消动作。`available` 目前只检查下单能力，
  但会员页在这个状态下会显示可取消的文案；“能扣款却不能停止续费”仍是
  未关闭的契约缺口，不能靠页面文字宣称已有取消能力。
...
当前整合状态只见[交付面板](../../plans/active/continuous-world-delivery.md)。
