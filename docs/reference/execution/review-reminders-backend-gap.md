...
产品完整性工作树增加了发送服务就绪闸门：**仅有公开 VAPID key 不代表发送
服务已可用；没有明确就绪能力时，不申请通知权限、不新建订阅，说明当前不能
提供提醒。**已有订阅仍可读取和撤销。该闸门的假服务测试只验证客户端状态，
不能替代真实账号、后台调度和实体设备收到通知的证据。
本次进展见[产品完整性计划](../../plans/active/product-completeness.md)。
...
它只有“好”和“以后再说”。发送服务就绪且点击“好”才进入浏览器的
...
发送服务未就绪时进入明确的不可用说明，不能把权限已获准当成提醒已可送达。
