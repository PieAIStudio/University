...
正在学的材料说了一句话”这个域：两种模式共用同一份结构化意见、同一个控件和
同一条写路径，先尝试账号后端，后端不可用时再复制给现有 AI 工作流。按钮、字段
和即时回应由共享 `FeedbackNote` 决定，不因为目的地不同就复制一套模式实现。

当前真实状态是：

- `FeedbackPort` 已有 `submit` 和 `readMine` 契约；有账号后端时先尝试写入，后端
  缺席或报错时由同一个剪贴板适配器兜底；
- 后端提交成功后才会显示“收到。这条记在《课名》第 N 版上了。”；降级复制成功时
  明确显示没有送到系统但已经复制；后端和剪贴板都失败时保留输入框原话并显示失败；
...
`FeedbackPort` 不是 authoring / delivery 之间必须分开的第四个答案。两种模式共用
同一个提交控件、同一个 `FeedbackSubmission` 白名单和同一个写入编排：先尝试已有
账号后端；没有后端客户端时，或后端 insert 报错时，再调用同一个剪贴板适配器，把
`feedbackNote(...)` 拼出的完整文本复制出去。这里改变的是目的地，不是模式行为，
所以不能在 `AUTHORING` 分支里各写一条提交路径。

这条顺序故意把“学习者已经写下的字”放在系统故障之上。后端表尚不存在、权限未配好
或网络请求失败，都只能触发剪贴板兜底；如果复制成功，receipt 的 transport 是
`clipboard`，界面必须说清楚“没有送到系统，但已复制，可以贴给课程作者”，不能显示
“已收到”或打勾。只有后端失败且剪贴板也不可用或写入失败，提交才算完全失败；这时
`unavailable` 只描述连剪贴板都没有，不描述后端表尚未创建。输入框里的原话必须保留，
学习者可以重试或手动复制。

表建好之后，不是改写另一套模式分支，而是继续使用同一个装配：把现有账号客户端的
写入请求指向已应用并验证过的 `university.feedback` 表，完成 RLS 和真实会话 smoke
test；后端成功时才返回 `swimmer-backend`，UI 才显示“已收到”。剪贴板仍是故障兜底，
不能因表接通而删除。
...
- 模拟表不可用或权限拒绝，页面明确显示没有送到系统但已经复制到剪贴板；
- 再模拟剪贴板不可用，页面显示完全失败，输入框仍保留学习者原话且不显示对勾；
...
如果后端上线后 insert 或 RLS 出现问题，不要把交付端强行改成
`unavailable`：同一个 `FeedbackPort` 会把失败降级到剪贴板，控件继续显示，并明确
告诉学习者这次没有送到系统但已经复制。只有剪贴板也不可用时才显示完全失败；修复
后端后仍使用同一个装配即可，不需要迁移客户端进度。
...
全量读取、后端成功时的“已收到”浏览器证据、后端失败时的剪贴板降级证据、两处都
失败时保留原话的证据，以及一次确认答案/课文/邮箱没有进反馈请求的自动化测试。
未满足前，University 的诚实状态就是本文开头列出的状态。
