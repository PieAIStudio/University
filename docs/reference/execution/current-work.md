...
| Playable 3D learning arcade | [3D play-lab experiment](../../plans/active/interaction-3d-playlab.md), in `.worktrees/interaction-3d-playlab` / `codex/interaction-3d-playlab`. The catalog's `3D组件` group now contains six game-specific scenes and the three retained garden editions. The wax-island study was rejected and retired. The same lane now compares ten actual map props using original, soft-sculpted, physical-bevel and material-aware finishes; the nine games remain unchanged. The plan owns its scope and acceptance, separately from the parallel clay work. Keep the course lane and original 2D games intact; no publication or merge is implied |
