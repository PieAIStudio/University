...
| Playable 3D learning arcade | [3D play-lab experiment](../../plans/active/interaction-3d-playlab.md), in `.worktrees/interaction-3d-playlab` / `codex/interaction-3d-playlab`. The owner-approved visuals now serve actual shooting, falling and rising-word games in the catalog's `3D组件` group. The plan distinguishes current gameplay evidence from the superseded quiz prototype. Keep the course lane and original 2D games intact; no publication or merge is implied |
