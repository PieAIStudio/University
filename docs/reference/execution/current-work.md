...
| PRIMM lesson engine and step lessons | [Interaction-first experiment](../../plans/active/interaction-first-experiment.md); the only course lane is `.worktrees/interaction-first` / `codex/interaction-first` (learner-value-first absorbed and its worktree and branch deleted 2026-09-20, main merged in). PRIMM version 3 (one action per screen inside the five phases) is built; lesson one is landed in it; the other seven are version 2 until the production line writes steps. Do not publish without a separate instruction. |
