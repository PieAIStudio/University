...
| Learner-value-first comparison of the first five PRIMM lessons | [Interaction-first experiment](../../plans/active/interaction-first-experiment.md); Owner authorized this new `.worktrees/learner-value-first` / `codex/learner-value-first` lane from committed `ca2c7d3f`. Independent source storage and 23650/23651 preview keep the other AI's `interaction-first` work untouched. Useful task outcomes are designed before PRIMM. The plan owns checks and evidence; no publishing, pushing or mainline merge is authorized. |
