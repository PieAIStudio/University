...
| PRIMM lesson engine and micro-step lesson design | [Interaction-first experiment](../../plans/active/interaction-first-experiment.md); the only course lane is `.worktrees/interaction-first` / `codex/interaction-first` (learner-value-first absorbed 2026-09-20). A production line writes and lands PRIMM lessons natively; eight lessons are landed and played. The Owner-approved micro-step prototype is in journey v5; productizing it waits for the post-merge refactor. Do not publish without a separate instruction. |
