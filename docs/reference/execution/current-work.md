...
| Personal tasks, practice games and coverage checkpoints on the course map | [Map learning nodes](../../plans/active/map-learning-nodes.md); the Owner-authorized parallel worktree is `.worktrees/map-learning-nodes` / `codex/map-learning-nodes`. The local pilot keeps generated lessons private, distinguishes game practice from assessed skipping, and reuses native lesson/review identities. The plan owns preview boundaries and evidence; do not create another branch or change the first-five authoring lane |
