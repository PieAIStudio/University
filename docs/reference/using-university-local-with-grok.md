...
**鼠标停在词上**（不是点击）就会浮出释义卡：音标、词性、中文释义、用法、朗读。移开鼠标它自己消失。三条进入路径，因为读者有三种：

- **停住**——光标在词上静止约 0.1 秒才浮出。用的是「静止」而不是「进入」：这些词嵌在正文里，按「进入」算的话，光标每划过一个词都会弹一张卡，读一段话像放幻灯片。
- **点击 = 钉住**——钉住后鼠标可以离开，卡片不跟着走。卡片上有按钮，一个你伸手去够就消失的面板不算面板。再点一次取消钉住。
- **键盘聚焦**——Tab 到词上同样打开。这是键盘用户**唯一**的路径；纯悬停会把那三个词汇按钮永久锁在他们够不到的地方。

关掉的方式有四种：移开鼠标、按 Esc、点卡片右上角的 ×、点页面别处。从词斜着移向卡片上的按钮时，中途会离开词、还没进入卡片——这一段由「安全多边形」护着，卡片不会在你伸手的路上消失。

**朗读只用本机语音**（`localService === true`）。系统里没装英语语音时按钮置灰并说明原因，不联网合成——这和课程内容里外部图片被拦是同一条边界：页面自己不往外送要渲染的字。

**声音是挑过的，不是碰上的。** macOS 装了约 40 个英语语音，`getVoices()` 按字母序返回，而字母表的开头全是 1980 年代的搞怪语音：Albert、Bad News（葬礼调）、Bahh（羊叫）、Boing、Jester、Zarvox。所以「取第一个英语语音」会稳定地取到 **Albert**，而真正的系统嗓音 Samantha 排在第 31 位，永远轮不到。现在的做法是先剔除这份搞怪名单，再按「Premium/Enhanced 变体 > 各平台默认嗓音 > 语言地区」排序。

排序不可能在每台机器上都对，而能听见结果的只有你：英文模式打开后，右侧「本课英文词」面板顶部有**朗读声音**下拉框，选了会立刻试读并记住（存在 `localStorage`，按 `voiceURI` 记，因为语音列表的顺序在不同会话之间不稳定）。

想要更好的音质，可以在系统设置里装苹果的高质量语音包（**系统设置 → 辅助功能 → 朗读内容 → 系统声音 → 管理声音**，找带 Premium / Enhanced 的英语声音）。这一步得你自己做——改系统设置不在 AI 的权限里。

### 本课英文词面板

英文模式打开后，右侧证据栏下面会多一块面板，列出这节课标注的全部英文词、中文释义和各自的状态（复习中 / 认识 / 已掌握 / 已暂停），顶上是「N 个词 · 已处理 M」。点任意一个词会滚动到它在课文里的位置并聚焦。

在它之前，打开英文模式只改变正文，要回答「这节课一共几个词、我处理过哪些」只能把每条下划线都悬停一遍。
