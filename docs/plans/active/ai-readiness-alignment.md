...
The owner approved the 13-area diagnosis and full repair on 2026-09-12.
Work on the existing main checkout; its starting commit is `2316755f`.
Course, product and visual branch commits are already merged. Their retained
worktrees and dated media are preservation surfaces, not current task owners.

This phase aligns working conditions, not product design. The subsequent
UIKit 2.6.1 dependency upgrade and CTA migration is a separate work unit with
its own before/after browser evidence, tests and commits. Do not downgrade the
currently newer UIKit to silence the outdated portfolio pin.
...
Do not delete worktrees or real studies, read secret values, expand host
permissions, modify SwimmerUIKit, or bypass hooks. Source and dependency changes
need separate evidence from configuration presence. PGS's pre-existing edits
remain protected; only the approved repair delta belongs to this task.

Local receipts: `.devspace-reports/pgs-kit-alignment-20260912/`.
Browser evidence: `.devspace-visual/pgs-kit-alignment-20260912/`.
Current versions and selections live in package/lock/asset files, not this plan.
