...
App、测试和完整图文证据已形成实际提交 `283b9261cf04514f16e7786cb921d05b2d82b98b`，此前散落的新增文件一并纳入，提交前后的文档与提交消息检查均通过。后续交接说明修订位于其后的提交；取得版本时以 `codex/product-ux` 的实际 HEAD 为准。主线、其他工作树和上线配置没有随本批改写。

| 收尾项目 | 结果与边界 |
| --- | --- |
| 文件落盘与提交 | 代码、测试、现行说明、原始截图和最终对照已提交；没有把课程输入、学员数据库、`.env` 或构建目录混进本次提交 |
| 可独立复跑的产品入口 | 新的 `E2E_ONLINE_PORT=21898 pnpm e2e:product` 已实际运行：33 项通过，退出 0；不是只列出测试 |
| 本分支发布版本输入 | 四学科导出新鲜度通过；没有删 `activities` 或改松校验来接受未来课程源 |
| 全仓检查 | 全部模块单测、编译、静态与产物检查已执行到源课新鲜度；默认指向未来课程副本时仍被 `browser-ai.activities` 拒绝。不能写成整条 verify 全绿 |
| 默认双模式检查 | 使用匹配的四学科输入后，作者端读课恢复；进一步跑出了下面的地图/一致性/旧探针失败，确认重复失败后停止该诊断轮。它不是默认 E2E 全绿证据 |
| 发布 | 未发布、未开启真实收费；推送分支也不能代替最终联合验收。远端状态须用 Git 引用实际核验 |

完整双模式诊断新增交接项：`F.island-pick` 作者端边缘卡片方向；`G.avatar` 点击后就位时长；`G.one-chrome` 两种模式采集到的学习控件不一致；`M.course-accessibility` 减少动效下实例矩阵仍变化。`M.experience` 还在按旧练习标题、常驻登录表单及会员说明弹层寻找控件，与已批准的短文案、展开表单、购买内联失败不一致。整合时应先对齐两种模式的课程上下文和真实媒体偏好，再逐项查因；旧探针要调整采集动作，同时保留实际命中、尺寸、响应和退出断言，不能整体删除或新增豁免。此列表是已复现的重点，不宣称已经穷尽所有联合问题。
