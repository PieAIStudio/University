---
id: ARCHIVE-CURRENT-WORK-2026-09-07
title: Current Work Before Consolidation 2026-09-07
type: archive
status: archived
canonical: false
owner: human
...
> Historical snapshot, not startup instructions or current runtime state. The
> active index is [Current Work](../../reference/execution/current-work.md).
> Old process/model choices, test totals and `pnpm content` advice below must
> not override the current project baseline. Retained for evidence, not execution.
...
> **3D 与课程分线工作。** 3D 在 sibling worktree `University-3d` 的
> `codex/continuous-island`；课程由另一条工作线维护。主线与旧工作区均须保留，
> 接续前重新检查实际分支、改动和进程，不从历史回执推断文件所有权。
>
> **三层是同一来源的不同投影。** 课程岛使用连续地形和实际三角面采样；系列岛群
> 使用低成本远景投影；最高层按领域绘制球体，系列是其大气层区域，不再是放大的
> 平面岛。Map Studio 读取实际投影与资产记录。架构与技术选择遵循
...
> 三层地图的美术方向与当时设计见 [V5](../../reference/player-journey/v5/index.html)。
>
...
这些启动命令以依赖、core 构建和内容链接已经就绪为前提；新 worktree 先按
[项目 baseline](../../policy/best-practice-for-this-project.md#working-in-a-worktree)
准备。不要把 `pnpm content` 当成修复预览或启动页面的步骤。
...
**2026-09-01 delivery shelf 快照：4 studies · 44 courses · 128 units · 495 lessons**
