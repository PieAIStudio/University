...
/**
 * Thirteen tree cards and two bush cards make a readable cluster on desktop.
 ...
 * Local Playwright Chromium measurement (headless SwiftShader, unfrozen rAF,
 * five-second warmup, two-second sample) is 5.34 FPS at 1440 by 900 with 660
 * cards; the 390 by 844 mobile tier is 20.40 FPS with 360 cards.
 */
export const ISLAND_CARD_VEGETATION_LEAF_COUNTS = {
  desktop: { tree: 13, bush: 2 },
  mobile: { tree: 9, bush: 1 },
} as const;
...
function normaliseLeafGeometry(source: SourceMeshDescriptor): THREE.BufferGeometry {
  ...
  for (let index = 0; index < position.count; index += 1) {
    ...
    const normalizedY = (position.getZ(index) - box.min.z) / height;
    const crownTaper = 0.68 + 0.32 * Math.sin(Math.PI * normalizedY);
    // The source is a narrow upright card. A modestly wider silhouette gives
    // the same two-triangle idea a canopy-like read without giant near-camera
    // discs.
    positions[offset] = ((position.getX(index) - centreX) / height) * 1.1 * crownTaper;
...
const CARD_VERTEX_NORMAL = [
  ...
  "vec3 leafCardOrigin = ( modelMatrix * vec4( 0.0, 0.0, 0.0, 1.0 ) ).xyz;",
  "#ifdef USE_INSTANCING",
  "  leafCardOrigin = ( modelMatrix * instanceMatrix * vec4( 0.0, 0.0, 0.0, 1.0 ) ).xyz;",
  "#endif",
  "vec3 leafToCamera = cameraPosition - leafCardOrigin;",
  "leafToCamera.y = 0.0;",
  "float leafToCameraLength = length( leafToCamera );",
  "vec3 leafFacing = leafToCameraLength > 0.0001",
  "  ? leafToCamera / leafToCameraLength",
  "  : vec3( 0.0, 0.0, 1.0 );",
  "vec3 leafRight = vec3( leafFacing.z, 0.0, - leafFacing.x );",
  "#ifndef FLAT_SHADED",
  "  // The card is billboarded around Y, so its lit normal must turn with it too.",
  "  vNormal = normalize( ( viewMatrix * vec4( leafFacing, 0.0 ) ).xyz );",
  "#endif",
].join("\n");

const CARD_VERTEX_PROJECT = [
  CARD_SHADER_MARKER,
  "#ifdef USE_INSTANCING",
  "  float leafCardScale = length( instanceMatrix[ 0 ].xyz );",
  "  float leafCardTurn = atan( instanceMatrix[ 0 ].z, instanceMatrix[ 0 ].x );",
  "#else",
  "  float leafCardScale = 1.0;",
  "  float leafCardTurn = 0.0;",
  "#endif",
  "float leafCardCos = cos( leafCardTurn );",
  "float leafCardSin = sin( leafCardTurn );",
  "vec2 leafCardLocal = vec2(",
  "  leafCardCos * position.x - leafCardSin * position.y,",
  "  leafCardSin * position.x + leafCardCos * position.y",
  ");",
  "vec3 leafCardWorldPosition = leafCardOrigin",
  "  + leafRight * leafCardLocal.x * leafCardScale",
  "  + vec3( 0.0, 1.0, 0.0 ) * leafCardLocal.y * leafCardScale;",
  "vCardLocal = position.xy;",
  "vec4 mvPosition = viewMatrix * vec4( leafCardWorldPosition, 1.0 );",
  "gl_Position = projectionMatrix * mvPosition;",
].join("\n");

const CARD_VERTEX_WORLD_POSITION = [
  CARD_SHADER_MARKER,
  "vec4 worldPosition = vec4( leafCardWorldPosition, 1.0 );",
].join("\n");
...
const CARD_FRAGMENT_COLOUR = [
  CARD_SHADER_MARKER,
  "// The ramp is deliberately broad: height supplies the main read, while the",
  "// low-frequency patch keeps a canopy from becoming one flat green blob.",
  "vec3 cardLow = vec3( 0.04, 0.16, 0.018 );",
  "vec3 cardHigh = vec3( 0.22, 0.46, 0.065 );",
  "vec3 cardColour = mix( cardLow, cardHigh, smoothstep( 0.32, 0.98, vCardHeight ) );",
  "cardColour *= mix( 0.76, 1.18, vCardNoise );",
  "vec2 cardShape = vec2( vCardLocal.x * 1.1, vCardLocal.y - 0.5 );",
  "float cardAngle = atan( cardShape.y, cardShape.x );",
  "float cardEdge = 0.53",
  "  + 0.035 * sin( cardAngle * 3.0 + vCardNoise * 6.2831853 )",
  "  + 0.02 * sin( cardAngle * 7.0 - vCardNoise * 4.0 );",
  "if ( length( cardShape ) > cardEdge ) discard;",
...
function createLeafMaterial(): THREE.MeshStandardMaterial {
  ...
  material.customProgramCacheKey = () => "island-card-vegetation-leaf-1";
...
function makeCardInstances(
  ...
  for (const placement of placements) {
    ...
    for (let cardIndex = 0; cardIndex < count; cardIndex += 1) {
      ...
      const radius =
        kind === "tree"
          ? foliageHeight * (0.1 + radial * 0.2)
          : foliageHeight * emitterRatio * (0.12 + radial * 0.24);
      const cardY =
        baseY + foliageHeight * (kind === "tree" ? 0.58 + vertical * 0.34 : 0.08 + vertical * 0.34);
      const cardScale =
        foliageHeight * (kind === "tree" ? 0.24 + random() * 0.08 : 0.26 + random() * 0.1);
  ...
  useLayoutEffect(() => {
    ...
    for (const [bucketIndex, bucket] of trunkBuckets.entries()) {
      ...
      for (const [instanceIndex, instance] of bucket.instances.entries()) {
        ...
        dummy.scale.set(instance.height * 1.4, instance.height, instance.height * 1.4);
        dummy.updateMatrix();
    ...
    for (const [index, card] of cards.entries()) {
      ...
      dummy.rotation.set(0, card.turn, 0);
