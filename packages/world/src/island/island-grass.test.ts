...
describe("Island grass plan", () => {
  ...
  it("uses the measured clump budgets and keeps the real course dense", () => {
    ...
    expect(plan.placements.every((placement) => placement.width >= 0.72)).toBe(true);
    expect(plan.placements.every((placement) => placement.width < 1.01)).toBe(true);
    expect(plan.placements.every((placement) => placement.height >= 0.72)).toBe(true);
  });
