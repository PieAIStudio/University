/**
 * Three.js adapters for the serialisable IslandBlueprint V2.
 *
...
import {
  ...
  type IslandOutlinePointV2,
  type IslandPointV2,
} from "./island-blueprint-v2.js";
import { hash } from "./random.js";

export type IslandGeometryV2Detail = "course" | "world";

export interface IslandGeometryV2Shape {
...
export function islandGeometryV2Scale(
  blueprint: IslandBlueprintV2,
  detail: IslandGeometryV2Detail,
  targetRadius?: number,
): number {
  return targetRadius
...
function sampleCount(detail: IslandGeometryV2Detail, outline: readonly IslandOutlinePointV2[]) {
...
function topRadials(detail: IslandGeometryV2Detail): readonly number[] {
  return detail === "course" ? COURSE_TOP_RADIALS : WORLD_TOP_RADIALS;
}

function outlineAt(
  outline: readonly IslandOutlinePointV2[],
  index: number,
  count: number,
): IslandOutlinePointV2 {
...
function topMeshVertex(
  ...
  if (radial === 0) {
    return { x: 0, z: 0, y: sampleIslandSurfaceV2(blueprint, 0, 0).y };
  }
  const point = outlineAt(blueprint.outline, index, segments);
  const x = point.x * radial;
  const z = point.z * radial;
  return { x, z, y: sampleIslandSurfaceV2(blueprint, x, z).y };
}

function barycentricHeight(
  point: IslandPointV2,
  first: TopMeshVertexV2,
  second: TopMeshVertexV2,
  third: TopMeshVertexV2,
): number | null {
  const denominator =
 ...
 *
 * `sampleIslandSurfaceV2` remains the canonical continuous authoring rule;
 ...
 */
export function sampleIslandTerrainTopV2(
  blueprint: IslandBlueprintV2,
  detail: IslandGeometryV2Detail,
  x: number,
  z: number,
): ReturnType<typeof sampleIslandSurfaceV2> {
  const continuous = sampleIslandSurfaceV2(blueprint, x, z);
...
function safePathSurface(
  ...
  readonly sample: ReturnType<typeof sampleIslandSurfaceV2>;
} {
  const direct = sampleIslandSurfaceV2(blueprint, target.x, target.z);
  if (direct.inside) {
    return {
      point: target,
      sample: sampleIslandTerrainTopV2(blueprint, "course", target.x, target.z),
    };
  }
  ...
  let bestSample = sampleIslandTerrainTopV2(blueprint, "course", centre.x, centre.z);
  for (let iteration = 0; iteration < 12; iteration += 1) {
    ...
    if (sample.inside) {
      low = amount;
      bestPoint = point;
      bestSample = sampleIslandTerrainTopV2(blueprint, "course", point.x, point.z);
    } else {
      high = amount;
    }
  }
...
function buildTerrain(
  ...
  const centre = sampleIslandSurfaceV2(blueprint, 0, 0);
...
export function buildIslandGeometryV2(
  blueprint: IslandBlueprintV2,
  detail: IslandGeometryV2Detail,
  targetRadius?: number,
): IslandGeometryV2Shape {
...
export function islandGeometryV2Key(
  blueprint: IslandBlueprintV2,
  detail: IslandGeometryV2Detail,
  targetRadius?: number,
): string {
...
export const ISLAND_GEOMETRY_V2_PALETTE = {
  grass: GRASS.getHex(),
