...
describe("probe", () => {
  it("reports route coverage", () => {
    ...
    for (const [study, course, n] of [
      ...
      const len = bp.centerline.reduce(
        (s, p, i) =>
          i === 0
            ? 0
            : s + Math.hypot(p.x - bp.centerline[i - 1]!.x, p.z - bp.centerline[i - 1]!.z),
        0,
      );
      lines.push(
        [
          `${course} n=${n} arch=${bp.route.archetype} halfX=${bp.bounds.halfX.toFixed(1)} halfZ=${bp.bounds.halfZ.toFixed(1)}`,
          `len=${len.toFixed(1)} lineCover=${coverage(bp, 0.22).toFixed(3)} nodeCover=${nodeCover.toFixed(3)} spanX=${spanX.toFixed(2)} spanZ=${spanZ.toFixed(2)}`,
        ].join(" "),
      );
    }
