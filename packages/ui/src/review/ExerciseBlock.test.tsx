...
describe("ExerciseBlock metered grading choice", () => {
  it("shows the cost and balance before an explicit paid choice, while keeping free tier one reachable", async () => {
    ...
    expect(container.textContent).toContain("只有点“使用 AI 批改”才会扣除额度");
    expect(buttonWith("只看免费提示（不花额度）")).toBeTruthy();

    await act(async () => {
      buttonWith("只看免费提示（不花额度）")?.click();
    });
    expect(container.textContent).toContain("已选择免费提示（不花额度）");
