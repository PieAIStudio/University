 ...
 *
 * Default on: that is the study-group plan's reason to exist. The switch
 * still has to work without a backend — refusing to be watched cannot wait
 * on a migration. The database column `study_group_members.shares_presence`
 * is the cloud copy of this same bit, and RLS only lets you change your own
 * row; until that table is deployed, localStorage is the whole store.
 *
