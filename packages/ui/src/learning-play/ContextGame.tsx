...
export function ContextGame({ activity, disabled, onAttempt }: ActivityControls<ContextActivity>) {
  ...
  const paperTop = useRef<HTMLDivElement>(null);
  ...
  function deliver() {
    ...
    onAttempt(
      ...
      passed
        ? translate("play.ai.context.customerSuccess")
        : resultStale
  ...
  return (
    <div className="play-ai-workflow play-ai-context">
      ...
      <div className="play-ai-context__desk" ref={paperTop}>
        <nav className="play-ai-context__library" aria-label={translate("play.ai.context.library")}>
        ...
        {selectedDocument ? (
          <section className="play-ai-context__paper" aria-label={selectedDocument.title}>
            <header>
              ...
              >
                {translate(
                  selectedDocument.paragraphs.every((paragraph) => selection.includes(paragraph.id))
            ...
            {selectedDocument.paragraphs.map((paragraph, index) => (
              ...
              >
                ...
                <GameToggle
                  ...
                  onClick={() => edit(toggleContextParagraph(activity, selection, paragraph.id))}
                />
              </div>
            ))}
          </section>
        ...
        {packed.length ? (
          <ul className="play-ai-context__packed-list">
            {packed.map(({ document, paragraph, index }) => (
              <li key={paragraph.id}>
                <span>
                  {document.title} · {translate("play.ai.context.paragraph", { count: index + 1 })}
                </span>
                <GameButton
                  variant="ghost"
                  sound={false}
      ...
      <section className="play-ai-context__result" id={resultId}>
        ...
        {resultStale ? (
          <div className="play-ai-workflow__warning" role="status">
            ...
            <GameButton variant="secondary" sound={false} disabled={disabled} onClick={generate}>
