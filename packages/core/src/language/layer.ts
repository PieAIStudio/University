...
/**
 * Composes the foreign-language layer a lesson is read with.
 *
 ...
 *
 * This module is pure on purpose. The authoring shell reads the overlay off
 * disk and the lexicon off disk; the delivery shell has neither a disk nor a
 * server, and ships the lexicon in its bundle. Both arrive here as arguments,
 * so the rule for which words a learner sees is one implementation rather than
 * two that drift.
 */
...
/** A hand-authored overlay, already read from wherever it lives. */
...
export function composeLanguageLayer(input: {
  ...
  /**
   * Absent means "nobody annotated this lesson by hand", which is the normal
   * case, not a degraded one — detection covers every lesson containing a word
   * the lexicon knows.
   */
  readonly authored?: AuthoredOverlay;
  ...
  const authoredAnchors: LanguageAnchor[] =
    input.authored?.status === "annotated"
      ? input.authored.ranges
          .filter((range) => {
  ...
  const detected = remaining
    ? detectAnchors(
        input.content,
        input.lexicon.filter((entry) => !covered.has(entry.senseId)),
  ...
  return {
    // "annotated" means "has a layer", which is every lesson that contains a
    // word the lexicon knows.
