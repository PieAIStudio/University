...
const EXPECTED_FLOW_STEPS: Readonly<Record<string, number>> = {
  "一次改动，从你的电脑到线上": 10,
  "一次网页请求，从你输入网址到看见页面": 9,
  "一次界面渲染，从文件到屏幕": 7,
  一次表单提交: 7,
  "一次 AI 请求": 9,
  一次测试: 7,
...
describe("concept catalogue", () => {
  ...
  it("keeps each flow on exactly the designated concepts", () => {
    ...
    for (const entry of flowEntries) {
      ...
      expect(section.payload.steps).toHaveLength(EXPECTED_FLOW_STEPS[section.payload.title]);
