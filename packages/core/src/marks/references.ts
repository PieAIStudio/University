import type { LessonLinkTarget } from "../domain/lesson-marks.js";
import type { LessonSection } from "../domain/schemas.js";
import { findProtectedRegions } from "../language/resolve-anchors.js";

export type { LessonLinkTarget } from "../domain/lesson-marks.js";
...
/** One lesson's bytes, enough to resolve its outgoing links without touching disk. */
export interface LessonIndexSource extends LessonLinkTarget {
 ...
 * directory order.
 *
 * The authoring shell supplies lessons by reading disk; the delivery shell
 * supplies the course it has already loaded. The index is the same either way.
 */
export function assembleLessonIndex(lessons: readonly LessonIndexSource[]): LessonIndex {
...
export function resolveLessonLinks(
  ...
  // Other token kinds (`evidence:`, `term:`) have their own resolver. Judging
  // them here would report every evidence anchor as a broken lesson link.
  return links
