...
继承 `apps/local/.agents/skills/write-lesson/` 的 house voice，但先区分词条正文和
链路图：它们是两种阅读场景，不能共用一条比喻规矩。

- `flow` 是地图，不是课文。每一步只有一个站名和一句说明：`label` 简短，
  `description` 一行、一句、不打比方、不用分号。每一步只描述这一步本身，不能把
  所有高亮到这一步的词一起塞进来。`description` 不得超过 40 个字符（中文字符按
  1 个计），也不得包含 `；` 或 `打个比方`；根检查脚本会机械拦截这些违规。
- 类比属于词条正文的十二段。正文有空间先说字面机制，再用 `打个比方：` 展开；
  删掉类比后字面层仍然成立。不要把正文的类比搬进 flow，也不要让一张地图同时
  承载十个比喻。
- 普通 flow step 的 `description` 仍然是一句话，短句优先；schema 上限是 500
  字符，`label` 上限是 80 字符，但上限不是目标。
