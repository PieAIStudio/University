---
id: ARCH-CURRENT-WORK-RECEIPTS-2026-08
title: Current Work Receipts (2026-07-20 to 2026-08-08)
type: archive
status: archived
canonical: false
owner: human
...
# Archived Current Work Receipts

This file preserves the detailed implementation and proof receipts that used to
live in `docs/reference/execution/current-work.md`. It is historical evidence,
not the current project queue or agents-routing algorithm.

## Current Focus

- Current phase: harden the local teaching system so an AI host can resume the
  right lesson without guessing, current course content can be recovered from
  Git, and a moved source checkout can be rebound without losing study or
  learner state.
- Current tracked plan/spec: none. This section records current truth, not an
  untracked implementation diary.
- Current proof target: `teach next` agrees with the Web Today view; a canonical
  course export round-trips through an empty studies root; snapshot creation
  rejects secret-like tracked paths; and source rebind accepts only a checkout
  that proves every stored snapshot commit and tree.
- Teaching-variant migration is complete for the current active lessons. Do not
  copy a count into this file; `pnpm lint:lessons` and the study manifests are
...
- Keep UniversityLocal on the portfolio SwimmerUIKit baseline declared in
  `package.json`; do not duplicate the current version number in this file.
...
- Template collapse strips code-like tokens and counts distinct prose skeletons.
  Stripping _all_ ASCII was rejected: it leaves an empty skeleton for every
