...
这份文档保存教学方法的依据与当时的取舍；实际写课合同由
[`write-lesson`](../../.agents/skills/write-lesson/SKILL.md)唯一维护，不由本文另定结构。

2026-09-09用户确认：每节必须有一道有开场依据的预测，具体正常任务也可作为开场，不强制反常。
“可答”是能作有理由的初步判断，不是已经知道答案。五种变体按内容选，连续同型的合理例外由
评审说明；机器不替人判断例外。自检换条件但不印答案，独立练习在正文下方判分并反馈；
演示型互动件不能替代独立练习。无仓库使用核实的权威材料与真实场景，不发明代码锚点。
这些是本产品的编辑与教学约定，不能从下述研究效应量推出“固定标题保证所有课程最优”。
...
```
标题（问句）→ 具体情境/任务 → 先猜一下 → 答案 → <中段> → 再想想（可选）→ 自检 → 一句话
```
...
| 变体 | 什么时候用 | 中段 |
| --- | --- | --- |
| `现象` | 有可观察的情境或具体任务，不要求意外 | 解释原因或示范完成任务 |
| `对比` | 两个东西老搞混 | `## 逐条对照` + `## 什么时候用哪个` |
| `溯源` | 信息或动作跨文件／真实系统流动 | 沿有依据的路径逐站追踪 |
| `决策` | 有取舍要讲 | `## 代价和收益` + `## 什么时候该反过来` |
| `术语` | 一个词总被读错 | 三个真实场景的白话用法 + `## 它不是什么` |
