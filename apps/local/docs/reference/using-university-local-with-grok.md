...
常见 study：`supaluv`、`turing-pact`、`university-local`（自学本仓，源是 airlock）。
把上面那句话里的名字换掉即可切换学习对象。
...
`学习项目` 页的左栏分两段。上面是**正在学习中**，最多三个，按你最近真的动过的时间排；下面是完整列表，保持固定顺序。打开页面时默认选中的是上面第一个，也就是你上次在学的那个。

在它之前，默认选中的是 `studies[0]`——也就是标题排最前的那个。于是每次进来第一件事都是先离开它。

「最近」是从学习事件里**推导**的，不是另存一个「上次打开时间」字段：取该 study 的 `learning.sqlite` 里复习、课时进度、练习、回忆四张表的最大时间戳。从没学过的 study 不会出现在这一段——它不是排最后，是不属于「正在学习中」。用推导而不是存字段，是因为存字段就多了一个可以和事件对不上的事实来源；而要你自己维护一份 pin 列表，等于让你把系统已经看着你做完的事再说一遍。

下面那份完整列表**故意不跟着动**。一个会自己重排的列表每次都得重新读一遍；让上面那段吸收全部变化，下面那份就能一直待在你记得住的位置。
...
这两处看着都是「答题」，但持久化方式**故意不同**，因为它们考的不是一回事。

**课程练习**：你提交过的答案会回填到输入框，下面写着提交时间。它一直存在 `learner/learning.sqlite` 里；从前只是页面每次从空白开始，所以看起来像丢了。通过之后输入框变只读，旁边多一个**重新回答**按钮——按下去可以重写重交。重答不会撤销已通过的记录，也不会让课时退回未完成：练习是练，不是考试，重做一遍只有好处。点「放弃重答」会把服务器上那份答案原样放回来，所以退出重答这个动作本身不会弄丢东西。

**复习卡（「先想，再看答案」）**：输入框**每次都是空的**，而且必须是空的。这是 FSRS 间隔重复卡，几天后还会再问你一次；如果打开就看见自己上次写的答案，回忆这一步等于被跳过了——那这张卡就白排了。打个比方：这就像一张答案被隐约印在正面背后的抽认卡，你会以为自己记得。

你写过的每一条**都存着**，只是出现的时机不同：点「揭示答案」之后，参考答案下面会列出「你以前答过 N 次」和当时的日期。看见上个月自己写得更含糊，正是有用的那部分。这个时机是**服务端**保证的——历史只随揭示接口返回，其他任何接口都拿不到它，所以不是靠前端记着别提前显示。卡片被改写过的话，旧答案会标「答的是旧版」。

揭示后还有一个**让 AI 讲讲这张卡**按钮，把问题、参考答案、你这次和以前的回答打成一个包复制走。包里明写着要**讲解、不要判分**：下面那四个按钮（重来/困难/良好/简单）问的是「你回忆得费不费劲」，只有你答得了；把外人的判断填进去，等于让别人的记忆来安排你的复习。
...
读课文时顺带认词。默认**关闭**。开关在课程页顶部，文案是 **外语模式 · 开/关**。
**有可教的词才出现开关**——词来自「手写标注 ∪ 词表自动检测」，不是「有没有人手标过」；
一课若词表一个也匹配不上，就不显示空开关。

它是**旁注层**，不改课文一个字节。入口：

- 合成层：`server/language/layer.ts`（手写 overlay 优先，检测补齐预算）
- 检测：`server/language/detect.ts`（只在散文里找词表 headword；代码区永不标）
- 预算：`adaptiveTargetCount`——从 3 起，随你「认识」的词变多，饱和到 12
- 词表空洞报告：`scripts/vocabulary-coverage.mjs`（课里出现但词表解释不了的词）

手写 overlay 仍可选，按 `contentRevision` + 内容哈希绑定；哈希对不上时该 overlay
作废，**检测仍可从当前课文推出一层**，而不是整层消失。开关外语模式**永远不会**
影响完成状态和复习排期——完成与排期绑在课文 revision 上，旁注不进那条账。

**鼠标停在词上**（不是点击）就会浮出释义卡：音标、词性、中文释义、用法、朗读。
移开鼠标它自己消失。三条进入路径：

- **停住**——光标在词上静止约 0.1 秒才浮出（避免划过整段时连环弹层）。
- **点击 = 钉住**——再点取消。
- **键盘聚焦**——Tab 到词上同样打开。

关掉：移开鼠标、Esc、卡片上的 ×、或点页面别处。从词斜着移向卡片按钮时有安全区，
卡片不会在半路上消失。

**朗读只用本机语音**（`localService === true`）。没装英语语音时按钮置灰，不联网合成。
macOS 上会先剔除 Albert / Bahh 等搞怪嗓音，再按 Premium/Enhanced > 系统默认排序。
外语模式打开后，右侧词面板顶部有**朗读声音**下拉框，选了会试读并记住
（`localStorage`，按 `voiceURI`）。

### 本课词面板

外语模式打开后，右侧证据栏下会多一块面板：本课词、中文释义、状态
（复习中 / 认识 / 已掌握 / 已暂停），以及各自是「新词 / 学习中 / 已熟」的原因。
点一个词会滚到正文位置并聚焦。已掌握的词在正文里变淡，**不占预算名额**。

词库按**词义**索引，不按拼写：`commit.git` 与数据库里的 commit 是两条。

可选：给人写的课加手写标注（检测补不齐、或要钉死某次出现时）：
...
`occurrence` 从 1 起；`senseId` 必须在词表里。落在代码块、行内代码、链接、HTML、
表格里的锚点会被拒绝。定位是纯字符串查找，不是分词——`quote` 要够长或
`occurrence` 要指到独立那一次。

### 课文里的链接和证据锚点

课文 Markdown 可以带两种令牌（渲染层解析，不改磁盘上的含义之外的字节）：

| 令牌 | 作用 | 入口 |
| --- | --- | --- |
| `[[lesson:id]]` 或 `[[lesson:course/unit/id]]` | 跨课跳转；可选标签 | `server/content/lesson-links.ts` |
| `[[evidence:path:line]]` | 点到本课 manifest 已引用的源码行 | `server/content/evidence-anchors.ts` |

链接在**服务端**解析：目标不存在会明显标坏，而不是静默当普通字。课末有
backlinks（谁链到了这一课）；跟链时有返回栈，可回到跳出前那一课。

证据锚点**只能落在本课 evidence 已声明的路径与行范围内**——manifest 钉的是不可变
快照，自由点任意行等于引用没人核对过的东西。范围外的锚点渲染为坏掉的标记。

### 一节课怎么写（给 AI / 作者）

操作规范是技能 **`.agents/skills/write-lesson/`**（五个变体、清单、跨课规则）。
研究依据在 `docs/reference/lesson-pedagogy.md`，不重复当第二真相源。

manifest 可选字段 `variant`：`现象` / `对比` / `溯源` / `决策` / `术语`。
没写 `variant` 的旧课被 `pnpm lint:lessons` **跳过**；写了的课必须过机械检查
（标题是否问句、固定小节名是否齐全、证据锚点是否出现等）。该命令挂在
`pnpm verify` 里。

### 一节课有真实地址

`/studies/:study/:course/:unit/:lesson` 在加载时解析、导航时写入、浏览器后退时恢复
（`src/url-state.ts`）。刷新不会丢回「今日学习」。这不是路由库；组件拆分之前就是
这份 parse/format。
...
界面上偶发的术语（FSRS、REV、外语模式、证据……）用 `src/Tip.tsx` + `src/glossary.ts`
统一释义；同一词不能在两个地方各写一套。
...
6. **本地缓存不是跨设备成绩册。** `learner/backups/` 能防误重置和数据库损坏；登录后学习进度会同步到同一份云端账户文档，未联网时先进入本地 outbox。完整 `studies/` 仍应由你放进 Time Machine 或明确选择的外部备份方案，因为源码、UA 和原稿不会随 learner sync 上传。
...
- 不上传源码、UA 图谱和原稿；学习账户数据使用共享云端同步。
- 不把本地 AI 宿主改成产品 API key；本地端的 AI 来源保持宿主/剪贴板路径。
- 不安装 Obsidian/黑曜石插件作为必需运行时。
- 不自制第二套记忆曲线；统一使用 `ts-fsrs`。
- 不用关键词比例假装自动判分，不显示没有足够数据支撑的掌握度和热力图。

本地端和在线端共享学习表面与账户数据；产品商业能力和作者工作流仍由各自壳负责，
不能把源码书架误当成 learner sync 的上传对象。
