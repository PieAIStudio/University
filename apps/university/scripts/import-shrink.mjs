 ...
 * source/asset/assessment bytes. Repository lessons retain the strict guard.
  ...
  for (let index = 0; index < oldUnits.length; index += 1) {
    ...
    for (let lessonIndex = 0; lessonIndex < previous.length; lessonIndex += 1) {
      ...
      if (
        ...
        return false;
      for (const key of ["evidence", "assets", "cards", "exercises"]) {
      ...
      if (!preservesPublicRevisionMaterials(before, after)) {
        throw new Error(
          `import-courses: refusing unexplained shrink for ${where} ` +
            ...
            "requires both hash-verified packages and unchanged lessons, sources, assets and assessments.",
        );
      }
      explained.push({
        course: where,
        ...
        reason: "new public-source revision; sources, assets and assessments preserved",
      });
    }
  }
  return explained;
