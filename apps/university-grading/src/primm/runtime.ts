    ...
    async grade(value: unknown, signal?: AbortSignal): Promise<ExerciseAttemptResult> {
      ...
      return once(
        ...
        async (jobSignal) => {
          ...
          const missingClockTime =
            lesson.activity.make.clockTimeCheck && !containsClockTime(work.finalWork);
          ...
          if (missingClockTime) {
            checks.push({
              criterion: 0,
              outcome: "fail",
              evaluation: lesson.activity.make.clockTimeCheck!.missing,
              extensions: [],
              evidence: { from: "missing", quote: "" },
            });
          } else
            for (const [criterion, requirement] of lesson.exercise.rubric.entries()) {
              const { object: check } = await structured.generate({
                model: PREVIEW_MODEL,
                maxTokens: 650,
                signal: jobSignal,
          ...
          // A definite format omission may fail fast; it can never bypass the
          // full semantic review to produce a pass.
          const decision = missingClockTime
            ? checks[0]!
            : combineCriterionReviews(checks, lesson.exercise.rubric.length);
