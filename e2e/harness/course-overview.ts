...
export async function courseOverviewEvidence(page: Page) {
  return page.evaluate(() => {
    ...
    for (const x of [box.min.x, box.max.x]) for (const y of [box.min.y, box.max.y]) for (const z of [box.min.z, box.max.z]) {
      const p = state.camera.position.clone().set(x,y,z).applyMatrix4(terrain.matrixWorld).project(state.camera);
      points.push({x:rect.left+(p.x+1)*rect.width/2, y:rect.top+(1-p.y)*rect.height/2, z:p.z});
    }
    ...
    const obstacles = [...document.querySelectorAll<HTMLElement>(".nav-rail, .counter-row, .app-shell__aside, .nextup, .tab-bar, .picked--left, .map-framing-tools, .hint:not(.hint--dismissed)")]
      .map(element=>({name:element.className,rect:element.getBoundingClientRect()}))
      .filter(({rect})=>rect.width>0&&rect.height>0)
      .filter(({rect})=>projected.left<rect.right&&projected.right>rect.left&&projected.top<rect.bottom&&projected.bottom>rect.top)
      .map(({name})=>name);
    return {
      geometryId:terrain.geometry.uuid,sceneId:state.scene.uuid,
      pointCount:points.length,projected,obstacles,
      entirelyVisible:points.every(p=>p.x>=rect.left&&p.x<=rect.right&&p.y>=rect.top&&p.y<=rect.bottom&&p.z<1),
      camera:state.camera.position.toArray(),fov:state.camera.fov,far:state.camera.far,
      distance:bag.mapControls?.getDistance(), range:[bag.mapControls?.minDistance,bag.mapControls?.maxDistance],
...
export async function waitForCourseFraming(page: Page): Promise<void> {
  await page.evaluate(async () => {
    let previous: number[] = [], streak = 0;
    const bag = window as any;
    for(let i=0;i<180;i++) {
      await new Promise(requestAnimationFrame);
      const next: number[] = bag.three?.camera.position.toArray() ?? [];
      streak = next.length === 3 && next.every((n,j)=>Math.abs(n-(previous[j]??Infinity))<0.001) ? streak+1 : 0;
      previous=next;
      if(streak>=12) return;
    }
...
export async function assertCompleteCourseOverview(page: Page) {
  ...
  expect(evidence.entirelyVisible, "Top AND natural root must fit, not an empty sky or partial coast").toBe(true);
  expect(evidence.obstacles,"The actual island bounding box must clear opaque chrome").toEqual([]);
  expect(evidence.pointCount).toBe(8);
