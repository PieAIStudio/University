  ...
  return page.evaluate(
    withProject((project) => {
      ...
      for (let i = 0; i < positions.count; i++) {
        points.push(
          project.projectWorldToViewport(
            scratch.fromBufferAttribute(positions, i).applyMatrix4(terrain.matrixWorld),
            state.camera,
            rect,
          ),
        );
      ...
      const obstacles = [
        ...document.querySelectorAll<HTMLElement>(
          ".nav-rail, .counter-row, .app-shell__aside, .nextup, .tab-bar, .picked--left, .hint:not(.hint--dismissed), .map-breadcrumbs",
        ),
      ]
