...
test.describe("L actual 3D asset inspector", () => {
  ...
  test("reads live medallion, footing, model dimensions and atomic assembly evidence", async ({
    ...
    const course = page.getByRole("combobox", { name: "预览课程", exact: true });
    ...
    const before = await page.evaluate(() => {
      const bag = globalThis as unknown as {
        three?: { scene: { getObjectByName(name: string): { uuid: string } | undefined } };
      };
    ...
    await page.screenshot({ path: join(OUTPUT, "island-footing-inspector.png") });
    ...
    await page.screenshot({ path: join(OUTPUT, "island-replacement-restored.png") });
    writeFileSync(
      join(OUTPUT, "receipt.json"),
      JSON.stringify({ replacement, before, after, console: console.errors() }, null, 2),
    );
    console.assertClean();
  });
});
