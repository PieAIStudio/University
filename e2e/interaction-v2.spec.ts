...
test("stored V2 remains valid; the five revised pilot lessons now use PRIMM", () => {
  ...
  // The five pilots are what this test was written to guard, and the conversion
  // has since continued past them — `start-a-new-context`, `keep-the-qualifier`
  // and `draft-is-not-send` carry PRIMM too. Assert the pilots are converted
  // rather than that they are the only ones, so writing the next PRIMM lesson
