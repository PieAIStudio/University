...
import {
  SHARED_LANDSCAPE_CASES,
  SHARED_LANDSCAPE_CATALOGUE_REASON,
  coursePathOf,
...
const cases = SHARED_LANDSCAPE_CASES.length > 0 ? SHARED_LANDSCAPE_CASES : [null];
...
test.describe("W shared landscape across real course identities", () => {
  ...
  for (const item of cases)
    test(
      item
        ? `${item.studyId}/${item.id}: shared art preserves this real course and its route`
        : "requires three real courses across two studies",
      async ({ page }) => {
        if (!item) {
          test.skip(true, SHARED_LANDSCAPE_CATALOGUE_REASON);
          return;
        }
        const errors = watchConsole(page);
        const path = coursePathOf(item);
        const output = join(
        ...
        expect(before.study).toBe(item.studyId);
        expect(before.course).toBe(item.id);
