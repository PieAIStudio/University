...
async function waitForPreparedDomains(page: Page, count: number) {
  await page.waitForFunction(
    (domainCount) => {
      ...
      if (
        !projection ||
        projection.domainCount !== domainCount ||
        !state ||
      ...
      return (
        projection.domains.every(
          (domain: { id: string }) =>
            state.scene.getObjectByName(`domain-planet-${domain.id}`)?.userData
              .planetAssetsReady === true,
        ) &&
  ...
  return page.evaluate(() => {
    ...
    return bag.__planetProjection().domains.map((domain: { id: string }) => {
      ...
      return {
        ...
        globe: globe.geometry.uuid,
        texture: globe.material.map?.uuid,
        islands: islands?.geometry.uuid ?? null,
      };
    });
  });
}
...
test.describe("O 多领域星球 · 合成边界夹具（非课程目录）", () => {
  for (const scenario of cases) {
    test(`${scenario.domains}领域 ${scenario.series}系列 ${scenario.width}px${scenario.empty ? " 长名称与空领域" : ""}${scenario.reduced ? " 减少动态" : ""}`, async ({
      ...
      await page.addInitScript(() => {
        ...
        new PerformanceObserver((entries) => {
          for (const entry of entries.getEntries())
            receipt.tasks.push({ start: entry.startTime, duration: entry.duration });
      ...
      const folder = join(
        ...
        ".devspace-visual",
        "domain-acceptance",
        `${scenario.domains}-${scenario.series}-${scenario.width}-${Number(scenario.empty)}`,
      );
      ...
      const receipt: Record<string, unknown> = {
        ...
        scenario,
        scope:
          "Synthetic catalogue in the actual PlanetPage/PlanetStage, not published lessons; ordinary real-catalogue route navigation is tested by G/N. Chrome viewport emulation is not a physical phone.",
      };
      ...
      let pressedAt = 0;
      await humanClick(page, progress, "use the DOM while domain resources prepare", {
        beforePress: async () => {
          pressedAt = await page.evaluate(() => performance.now());
        },
      });
      ...
      expect(
        inputMs,
        "cold resource preparation must not monopolize the real pointer path",
      ).toBeLessThan(300);
      ...
      const labels = await page.locator("[data-planet-domain-label]").evaluateAll((elements) =>
      ...
      for (const label of labels) {
        ...
        expect(
          label.top,
          "a complete label, not just its anchor, must be inside the clipped viewport",
        ).toBeGreaterThanOrEqual(label.clip.top);
      ...
      for (let i = 0; i < labels.length; i++)
        for (let j = 0; j < i; j++) {
          ...
          expect(
            ...
            "projected domain labels must not collide",
          ).toBe(true);
        }
      ...
      if (scenario.domains === 4 && scenario.series === 20 && !scenario.empty) {
        ...
        await page.mouse.up();
        await page.waitForFunction(
          (id) => (window as any).__planetProjection().selectedId === id,
          target!.id,
        );
        receipt.realGlobeClickMs = await page.evaluate(
          (start) => performance.now() - start,
          pressed,
        );
        expect(receipt.realGlobeClickMs).toBeLessThan(300);
        await expect(page.locator(`[data-study-id="${target!.id}"]`)).toHaveAttribute(
          "aria-pressed",
          "true",
        );
      }
      ...
      await humanClick(
        page,
        page.locator(`button[data-domain-id="${selectedDomain}"]`),
        "select a peer domain",
      );
      await expect(page.locator(`button[data-domain-id="${selectedDomain}"]`)).toHaveAttribute(
        "aria-pressed",
        "true",
      );
      ...
      if (scenario.empty) {
        ...
        const ids = await rows.evaluateAll((elements) =>
          elements.map((element) => element.getAttribute("data-study-id")),
        );
        expect(ids.every((id) => id?.startsWith(`fixture-series-${scenario.domains - 1}-`))).toBe(
          true,
        );
        ...
        await humanClick(
          page,
          page.locator(".planet-page__enter"),
          "emit the series navigation intent",
        );
      ...
      const beforeReturn = await page.evaluate(() => ({
        ...
        longTasks: (window as any).__domainInputReceipt.tasks,
      }));
      ...
      await humanClick(
        page,
        page.locator("[data-fixture-mount]"),
        "return to the same selected domain",
      );
      await waitForPreparedDomains(page, scenario.domains);
      await expect(page.locator(`button[data-domain-id="${selectedDomain}"]`)).toHaveAttribute(
        "aria-pressed",
      ...
      expect(returned.map((domain: any) => domain.position)).toEqual(
        initial.map((domain: any) => domain.position),
      );
      for (let i = 0; i < initial.length; i++)
        expect(returned[i]!.globe).not.toBe(initial[i]!.globe);
      receipt.returned = await page.evaluate(() => ({
        frame: (window as any).__stageFrameMetrics,
        preparation: (window as any).__planetProjection().preparation,
      }));
  ...
  test("worker failure keeps the real DOM usable and retry reaches actual scene readiness", async ({
    page,
  }) => {
    ...
    expect(
      await page.evaluate(
        () =>
          (window as any).three.scene.getObjectByName("domain-globe-fixture-domain-0").material.map
            ?.image.data.length,
      ),
    ).toBe(1024 * 512 * 4);
  });
