...
- `path-albedo.webp`：奶油色沙石，低对比度、没有砖缝和方向性铺装图案。
...
- shader 使用普通隐式导数的 `texture()` 采样，让每个世界空间投影得到正确的 mip
  选择，不使用会锁死细节层级的 `textureLod`。
...
四张方形源图，再用仓库内的确定性脚本做接缝修复、低频亮度压平、调色和 WebP 编码。
...
1. Gemini 探测不到 key，不能继续假设 Gemini 可用；改走内置图像生成并在报告中留下
   这个事实。
2. 第一版岩石源图的横向层理太规则，像条形码；第一版 detail 又几乎没有可读变化。
   两者都被我在打开图片后否决，分别重新生成了更破碎的层理和更柔和的灰度细节。
3. AI 输出即使写着 seamless 仍需要实际做半幅 offset、中心/外圈周期接缝修补；验证
   指标必须在最终 WebP 解码后计算，不能只测压缩前的内存图。
4. 先用无损 WebP 时总大小为 525,366 bytes，超过 500 KB；在视觉检查没有被破坏的
   前提下改为质量 88 的有损 WebP，最终总大小降到 37,034 bytes。

资源已放在新建的 `packages/world/public/island-textures/`：当前 package 没有
`public/` 目录，而独立 demo 的 Vite root 会以 `packages/world` 为根，这个位置既
能被 demo 当作静态资源读取，又不会修改 `packages/world/src/island/**` 或任何现有
岛渲染文件。最终四张文件均为 512×512 WebP；大小、最终解码指标和 sha256 记录在
`island-texture-processing.json`，总资源 37,034 bytes，远低于 500 KB。
...
1. [4×4 平铺验证图](island-texture-tiling-4x4.png)：四个材质区域均没有肉眼可见
   接缝；岩石使用第二版源图后也没有第一版那种规则条形码重复感。
2. [8×8 亮度压平验证图](island-texture-flatness-8x8.png)：四块缩小后的色块没有
   暗角或方向性渐变。最终解码后的亮度范围分别为 grass 8.8568、route 3.0000、
   rock 5.7060、detail 2.0000。
...
4. [远景 demo 截图](island-texture-demo-far.png)：纹理缩小后仍是柔和色块，没有看到
   ...
   按屏幕像素分配预算的原则。
5. 同一 URL、同一 seed `17` 在两个独立浏览器会话中的画布 hash 都是
   `d8d3fe59`。demo 和纹理处理脚本均没有使用 `Math.random()`。
...
| --- | --- | ---: | ---: | ---: |
| 改前：无贴图 + 多材质 fallback | hybrid | 0.099 ms | 1 | 25,472 |
| 改后：四张贴图 + 多材质 | hybrid | 0.162 ms | 1 | 25,472 |
| 改后：四张贴图 + 单材质 | hybrid | 0.154 ms | 1 | 25,472 |
| 改后：四张贴图 + 多材质 | full triplanar | 0.256 ms | 1 | 25,472 |

所以贴图本身在这台机器上增加约 0.063 ms GPU render time，但不增加 draw call 或
三角形；hybrid 比 full triplanar 低约 36.7% 的 GPU 时间，且 4×4/近远景检查没有
...
独立页面 `packages/world/texture-demo/`、四张静态资源和三组验证图片。模块不导入
